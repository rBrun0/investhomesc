...
const Modal = ({codigoImovel, fetchImoveis}: ModalProps) => {
    ...
    // setBuildingSavedData();
        ...
        const fetchSiteData = async () => {
            try {
                const resultData = [];
                const q = query(collection(db, "imoveis"), where("codigoImovel", "==", codigoImovel));
                ...
                setBuildingSavedData(resultData); 
              } catch (error) {
                console.error("Erro ao buscar dados:", error);
              }
    }

    console.log('dataaa',{buildingSavedData})
        ...
        async function onSubmit(data: ImoveisType) {
            ...
            try {
                ...
                if (querySnapshot.empty) {
                    ...
                    toast.error("Nenhum imovel para atualizar!");
                    console.log("Deu");
                    return;
                }
                ...
                if (!querySnapshot.empty) {
                    ...
                    await updateDoc(docRef, {
                        ...data,
                        uid: buildingSavedData?.[0]?.uid,
                        descricao: data.descricao,
                        imagensUrl: uploadedImages,
                        perfil: data.buildingProfile,
                    ...
                    toast.error("Nenhum imóvel encontrado para atualizar!");
                }
            }
            catch(err) {
            console.log({err})
        ...
        useEffect(() => {
          ...
          useEffect(() => {
            ...
            setValue("bairro", buildingSavedData[0]?.bairro)
            ...
            setValue("areaPrivativa", String(buildingSavedData[0]?.areaPrivativa))
